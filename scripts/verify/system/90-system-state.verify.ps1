...
$temporaryGuestPresent = [bool](Get-LocalUser -Name "guest.TEMP" -ErrorAction SilentlyContinue)
if ($temporaryGuestPresent) {
    Add-SystemWarning "Temporary guest account EXISTS. Remove if no longer needed."
}
...
if (-not $temporaryGuestPresent) {
    ...
    Write-Info "Temporary guest account is present."
}
