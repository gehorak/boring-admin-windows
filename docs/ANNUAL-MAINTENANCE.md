...
Alternative Make-based entrypoint:

```text
make -f Makefile.audit audit
```

Optional software inventory visibility:
...
☐ Confirm no temporary access account remains active
...
☐ Confirm the external recovery location is still the intended escrow target
☐ Confirm administrator secrets remain stored in the documented external secret store
☐ Optionally run `.\boring-admin.ps1 verify recovery` and compare the result with the external recovery checklist
☐ Confirm the external checklist includes:
  * BitLocker recovery key storage location
  * administrator secret storage location
  * documented reinstall path
  * data recovery path for the primary user
  * current owner / operator contact
...
### 6. Temporary access account check

☐ Verify no temporary access account exists
