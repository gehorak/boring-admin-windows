...
try {
    $recoveryAdmin = Get-LocalUser -Name "admin.RECOVERY" -ErrorAction Stop
    $recoveryAdminState = [pscustomobject]@{
        exists  = $true
    ...
    Add-RecoveryWarning "Recovery administrator 'admin.RECOVERY' not found."
}
...
if ($recoveryAdminState.exists) {
    Write-Info "Recovery administrator exists: admin.RECOVERY"
...
Write-Info "- docs/OPERATING-MANUAL.md"
Write-Info "- offline recovery checklist or escrow record"
