...
# --------------------------------------
# - temporary access account from the active profile
...
try {
    Assert-BoringAdminWritableProfile -Config $config -Operation "Temporary local access management"
}
catch {
...
$TempUserName    = $identitySettings.temporaryAccessUserName
$TempDescription = $identitySettings.temporaryAccessDescription
...
switch ($choice) {
    ...
    "1" {
        if ($TempUser) {
            if (-not $TempUser.Enabled) {
                Write-Section "Apply: Enable temporary access"

                try {
                    Enable-LocalUser -Name $TempUserName -ErrorAction Stop
                    Write-Info "Temporary access account enabled: $TempUserName"
            ...
            Write-WarnFlagged "Temporary access account already exists and is enabled."
            break
        }
