...
### 1.4 Local Identity Boundary in Phase-1

This document governs the **local identity control surface** only.

It defines:

- local administrator ownership
- local recovery access
- local temporary access
- read-only visibility into local accounts and privileges

It does **not** define the complete end-user sign-in story by itself.

Daily user operation may still rely on a **cloud-backed personal identity**
as described in higher-level operational documents.

That cloud-backed user identity is an **operational assumption** of the wider system,
but it is **not managed by the local identity scripts in this document**.

---

### 1.5 What Local Identity Explicitly Does NOT Do (Phase-1)
...
- cloud / Microsoft account lifecycle management
- AD / MDM emulation
